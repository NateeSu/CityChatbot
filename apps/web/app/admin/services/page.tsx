...
export default async function ServicesAdminPage({ searchParams }: { searchParams?: Promise<{ role?: string }> }) {
  ...
  if (!identity || !serviceRole) return <main className="shell"><PermissionDeniedState action={<Link href="/admin">กลับหน้าหลัก</Link>} /></main>;
  ...
  return <ServiceConsole identity={identity} initialSnapshot={servicesRepository.snapshot(actor)} />;
}
