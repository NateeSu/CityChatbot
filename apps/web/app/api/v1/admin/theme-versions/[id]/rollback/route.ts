...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  if (!actor) return themeSettingsJsonError(403, "FORBIDDEN", "ไม่มีสิทธิ์ rollback Theme");
  try {
    const body = await readThemeSettingsObjectBody(request);
    const version = themeSettingsRepository.rollback(actor, (await params).id, themeIdempotencyKey(request, body), requiredThemeString(body, "reason"));
