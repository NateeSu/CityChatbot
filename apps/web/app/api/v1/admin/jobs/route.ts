...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try { return auditSuccess({ items: auditOperationsRepository.listJobs(actor), hasMore: false }, 200, requestId); } catch (error) { return auditDomainErrorResponse(error, requestId); }
}
