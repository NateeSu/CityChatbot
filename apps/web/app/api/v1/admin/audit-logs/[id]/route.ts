...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try { return auditSuccess(auditOperationsRepository.getAudit(actor, (await params).id), 200, requestId); } catch (error) { return auditDomainErrorResponse(error, requestId); }
}
