...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  if (!actor) return newsJsonError(403, "FORBIDDEN", "ไม่มีสิทธิ์อนุมัติข่าว");
  try { const body = await readNewsObjectBody(request); const post = newsRepository.approve(actor, (await params).id, newsExpectedVersion(request, body), newsIdempotencyKey(request, body), requiredNewsString(body, "reason")); return NextResponse.json({ post }); } catch (error) { return newsDomainErrorResponse(error); }
}
