...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  if (!actor) return botSettingsJsonError(403, "FORBIDDEN", "ไม่มีสิทธิ์เผยแพร่การตั้งค่า Bot");
  try {
    const body = await readBotSettingsObjectBody(request);
    const { id } = await params;
    const version = botSettingsRepository.publish(actor, id, botExpectedVersion(request, body), requiredBotString(body, "reason"), botIdempotencyKey(request, body));
