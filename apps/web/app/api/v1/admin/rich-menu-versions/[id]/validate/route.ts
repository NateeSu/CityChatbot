...
export async function POST(request: Request, { params }: Params): Promise<NextResponse> {
  ...
  if (!actor) return jsonError(403, "FORBIDDEN", "ไม่มีสิทธิ์ตรวจสอบ Rich Menu");
  ...
  try {
    ...
    const item = localRichMenuService.validate(actor.tenantId, id, actor, expectedVersion, typeof body.reason === "string" ? body.reason : "ตรวจสอบ Rich Menu", idempotencyKey);
    ...
    return jsonError(status, code, status === 500 ? "ไม่สามารถตรวจสอบ Rich Menu ได้" : error instanceof Error ? error.message.replace(`${code}: `, "") : "คำขอ Rich Menu ไม่ถูกต้อง");
  }
}
