...
export async function POST(request: Request, { params }: Params): Promise<NextResponse> {
  ...
  if (!actor) return jsonError(403, "FORBIDDEN", "ไม่มีสิทธิ์ย้อนกลับ Rich Menu");
  ...
  try {
    ...
    const item = await localRichMenuService.rollback(actor.tenantId, id, actor, expectedVersion, typeof body.reason === "string" ? body.reason : "ย้อนกลับ Rich Menu", idempotencyKey);
    ...
    return jsonError(status, code, status === 500 ? "ไม่สามารถย้อนกลับ Rich Menu ได้" : error instanceof Error ? error.message.replace(`${code}: `, "") : "ย้อนกลับ Rich Menu ไม่สำเร็จ");
  }
}
