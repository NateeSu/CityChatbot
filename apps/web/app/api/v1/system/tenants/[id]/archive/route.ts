...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try { const body = await readTenantSystemBody(request); const result = tenantProvisioningRepository.archiveTestTenant(actor, (await params).id, tenantExpectedVersion(request, body), requiredTenantString(body, "verificationText"), requiredTenantString(body, "reason"), tenantSystemIdempotencyKey(request, body)); return NextResponse.json({ tenant: result }); } catch (error) { return tenantSystemErrorResponse(error); }
}
