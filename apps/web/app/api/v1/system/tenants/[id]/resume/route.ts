...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try { const body = await readTenantSystemBody(request); const result = tenantProvisioningRepository.resumeProvisioning(actor, (await params).id, requiredTenantString(body, "reason"), tenantSystemIdempotencyKey(request, body)); return NextResponse.json(result); } catch (error) { return tenantSystemErrorResponse(error); }
}
