-- Eliminate cross-host clock races between Vercel and Postgres when LINE
-- webhook and delivery jobs are enqueued and claimed in the same invocation.
--
-- p_now remains supported for deterministic retry/lease advancement, while
-- claim_at never precedes the database clock that stamped next_attempt_at.
...
begin
  ...
  claim_at := greatest(p_now, statement_timestamp());
  lease_until := claim_at + make_interval(secs => p_lease_seconds);
  ...
  select j.* into selected_job
   ...
   where j.job_type = 'line.webhook.process'
     ...
     and (
       (j.status in ('QUEUED', 'RETRY_WAIT') and j.next_attempt_at <= claim_at)
       or (j.status = 'RUNNING' and j.lease_expires_at < claim_at)
     )
     and inbox.status in ('QUEUED', 'FAILED', 'PROCESSING')
     and (inbox.status <> 'PROCESSING' or inbox.lease_expires_at < claim_at or j.status = 'RUNNING')
  ...
  update public.jobs as job
     set status = 'RUNNING', attempt_count = job.attempt_count + 1,
         ...
         heartbeat_at = claim_at, started_at = coalesce(job.started_at, claim_at),
         updated_at = claim_at, row_version = job.row_version + 1
  ...
  update public.line_webhook_inbox as inbox
     set status = 'PROCESSING', attempt_count = inbox.attempt_count + 1,
         ...
         updated_at = claim_at, row_version = inbox.row_version + 1
  ...
  return query
    select selected_job.id, selected_inbox.id, selected_inbox.tenant_id,
      ...
      selected_inbox.event_timestamp, selected_inbox.webhook_event_id,
...
comment on function private.claim_line_webhook_job(text, timestamptz, integer)
  is 'Tenant-safe leased LINE webhook claim using a database-bounded clock to prevent same-invocation queue starvation.';
...
begin
  ...
  if p_lease_seconds < 5 or p_lease_seconds > 300 then
    raise exception using errcode = '22023', message = 'invalid LINE delivery lease';
  end if;
  ...
  select j.* into selected_job
    from public.jobs as j
    join public.line_messages as line_message
      on line_message.tenant_id = j.tenant_id
     and line_message.id = (j.payload_json->>'deliveryId')::uuid
   where j.job_type = 'line.message.delivery'
     and j.attempt_count < j.max_attempts
     and (
       (j.status in ('QUEUED', 'RETRY_WAIT') and j.next_attempt_at <= claim_at)
       or (j.status = 'RUNNING' and j.lease_expires_at < claim_at)
   ...
   order by j.priority desc, j.next_attempt_at, j.created_at, j.id
   for update of j, line_message skip locked
   limit 1;
  if not found then return; end if;
  ...
  select * into selected_message
   ...
   where line_message.tenant_id = selected_job.tenant_id
     and line_message.id = (selected_job.payload_json->>'deliveryId')::uuid
   for update;

  select * into selected_channel
    from public.line_channels as channel
   where channel.tenant_id = selected_message.tenant_id
     and channel.id = selected_message.line_channel_record_id
     and channel.state in ('ACTIVE', 'DEGRADED');
  if not found then return; end if;
  ...
  select * into selected_user
   ...
   where line_user.tenant_id = selected_message.tenant_id
     and line_user.id = selected_message.line_user_id
  ...
  update public.line_messages as line_message
     set status = 'SENDING', attempt_count = line_message.attempt_count + 1,
         updated_at = claim_at, row_version = line_message.row_version + 1
   where line_message.id = selected_message.id;

  return query
    select selected_job.id, selected_message.id, selected_message.tenant_id,
      selected_message.line_channel_record_id, lower(selected_message.route),
      selected_user.line_user_id, selected_message.reply_token_ciphertext,
      selected_message.reply_token_key_version, selected_message.content_ciphertext,
      selected_message.content_key_version, selected_message.idempotency_key,
      selected_message.correlation_id, selected_channel.encrypted_access_token,
      selected_channel.credential_key_version, selected_message.attempt_count + 1,
      selected_message.max_attempts, lease_until;
end;
$function$;

comment on function private.claim_line_message_job(text, timestamptz, integer)
  is 'Tenant-safe leased LINE delivery claim using a database-bounded clock to prevent same-invocation queue starvation.';
